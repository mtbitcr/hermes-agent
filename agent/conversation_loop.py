...
def _runtime_turn_block_message(agent: Any, effective_task_id: str) -> Optional[str]:
    """Return the first plugin runtime-admission block, if any (fork: pre_runtime_turn hook)."""
    try:
...
def _run_conversation_turn(
    ...
    # Fork admission gate: a plugin may block a runtime that would bypass a
    # security boundary that only exists in Hermes' own tool loop (used by the
    # Raphael sandbox run authority). Checked before any runtime handoff.
    runtime_block = _runtime_turn_block_message(agent, s.effective_task_id)
    if runtime_block:
