...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Fork policy (2026-09-12 sync): upstream retired this guard as ceremony;
    this fork keeps it — multiple profiles share one host here and the
    refusal-plus-``cross_profile=True`` flow is a shipped, tested behavior
    (file tools, patch, skill manager, system prompt all reference it).
