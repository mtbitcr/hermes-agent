...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Parallel repository work.** Fan out code only when slices are truly "
    "independent. Give every mutating child `workspace_kind=\"worktree\"` and "
    "explicit literal `owned_paths`; use `[]` for read-only work and `['.']` "
    "for exclusive whole-repository work. If scopes overlap, serialize them "
    "with `parents` instead of claiming false independence. Create one "
    "exclusive integration child, assigned to the approved integration "
    "responsibility, with `integrates_parent_heads=true`; it must depend on "
    "every builder and consume their exact kernel-provided git receipts. A "
    "changed parent head blocks completion. Follow it with a fresh read-only "
    "verification child that independently checks the exact combined head. "
    "Keep paths, branches, and commit IDs out of owner-visible prose.\n"
    "\n"
    ...
    "- **Remote sandbox.** Attach one repo-relative technical `.patch` and "
    "complete with `patch_attachment_id`; integration also sets "
    "`merge_parent_heads=true`. Never request a host mount.\n"
    "- **Deliverables.** Files a human wants go in "
    "`kanban_complete(artifacts=[<absolute paths>])` (top-level param; paths in "
    "`metadata` are NOT uploaded). Files must exist at completion. Attachments "
    "are owner-visible by default: unless the task explicitly requires a "
    "technical/operator artifact, use plain language and omit internal task/run "
    "IDs, agent/profile names, workspace/storage paths, branches, and commit "
    "SHAs. The attachment tools reject these references in ordinary text; "
    "use `technical_artifact=true` only when the task explicitly requires a "
    "technical or operator artifact. Keep operational evidence in the "
    "structured handoff instead.\n"
