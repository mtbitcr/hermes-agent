...
class HermesProviderMixin:
    ...
    def __init__(self, *args: Any, token_user_agent: str | None = None, oauth_flow: str = "browser",
                 claude_design: bool = False, callback_state: "dict[str, str | None] | None" = None,
                 **kwargs: Any):
        ...
        # Governed Claude Design route (fork): token and refresh requests are
        # rewritten against Anthropic's fixed endpoints; the authorization
        # callback state must be echoed into the token exchange.
        self._hermes_claude_design = claude_design
        self._hermes_callback_state = callback_state if callback_state is not None else {}
    ...
    async def _exchange_token_authorization_code(self, *args: Any, **kwargs: Any):
        ...
        request = self._prepare_token_request(await super()._exchange_token_authorization_code(*args, **kwargs))
        if not getattr(self, "_hermes_claude_design", False):
            return request
    ...
    async def _refresh_token(self):
        self._coerce_client_secret_post()
        request = self._prepare_token_request(await super()._refresh_token())
        if not getattr(self, "_hermes_claude_design", False):
            return request
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "HermesTokenStorage"]:
    ...
    cfg["_hermes_claude_design"] = is_claude_design
    storage = mo.HermesTokenStorage(server_name)
    # Pre-registration may invalidate stale tokens and metadata when an old
    # client ID is replaced. Seed after that migration so the native SDK never
    # falls back to Anthropic's retired discovery endpoint.
    if is_claude_design:
...
def build_provider_kwargs(cfg: dict, storage: "HermesTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
    ...
    if cfg.get("_hermes_claude_design"):
        # Governed Claude Design route (fork): retain the authorization
        # callback ``state`` so the rewritten token exchange can echo it.
        base_callback_handler = kwargs["callback_handler"]
        callback_state: dict[str, str | None] = {}
        ...
        kwargs["claude_design"] = True
        kwargs["callback_state"] = callback_state
    return kwargs
