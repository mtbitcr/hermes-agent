...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            enabled_toolsets=a["enabled_toolsets"] or None, max_turns=a["max_turns"],
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "max_turns": {
                ...
                "minimum": 1,
                "maximum": 500,
                "description": "Optional hard limit on agent turns per run. Use the smallest budget that can safely complete the job; omit to inherit the profile default."
            },
            "workdir": {
                "type": "string",
...
_HANDLER_FORWARDED_ARGS = (
    ...
    "script", "context_from", "continuity", "enabled_toolsets", "max_turns", "workdir", "no_agent", "attach_to_session",
    "paused_reason")
