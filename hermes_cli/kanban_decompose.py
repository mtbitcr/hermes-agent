...
def _clean_children(task_id: str, raw_tasks: list, routing: _Routing) -> tuple[list[dict], str]:
    ...
    for idx, entry in enumerate(raw_tasks):
        ...
        if workspace_kind is not None and workspace_kind not in kb.VALID_WORKSPACE_KINDS:
            return [], f"tasks[{idx}].workspace_kind is invalid"
        try:
            normalized_owned_paths = kb.normalize_owned_paths(entry.get("owned_paths"))
        except ValueError as exc:
            return [], f"tasks[{idx}].owned_paths is invalid: {exc}"
        integrates_parent_heads = entry.get("integrates_parent_heads", False)
        if not isinstance(integrates_parent_heads, bool):
            return [], f"tasks[{idx}].integrates_parent_heads must be a boolean"
        children.append({
