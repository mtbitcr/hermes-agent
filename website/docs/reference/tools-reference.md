...
| `kanban_review_findings` | Reviewer verdict as a typed findings document, valid only from the active reviewer run of that task (claimed from the review lane). The candidate is always the repository head the kernel parked at review handover; an explicitly empty findings array is the only thing that approves the task. | `HERMES_KANBAN_TASK` or `kanban` toolset |
...
| `kanban_receipts_summary` | Summarize past runs per boundary (the profile that ran them) across every board of the owner's store from their trusted run receipts, bounded by days (default 30, cap 90) and boards scanned (default 20, cap 200): runs by outcome, runs per provider, model and effort as the receipts record them, the coding tool's route requested by the card's current pin, which can differ from the pin a past run was dispatched with, on its own line labelled as requested, total run time, and cost as recorded (known amounts summed per currency with the runs they cover; unknown cost counted on its own line, never as zero). A run with no receipt, or a receipt of another shape, is counted as missing. Every board is opened read-only and nothing is written; what the bounds left out is disclosed. | Profiles named in `kanban.status_report_profiles`, their dispatcher-spawned task workers included; `delegate_task` children are refused |
