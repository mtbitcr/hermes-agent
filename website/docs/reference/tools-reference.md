...
| `kanban_status_report` | List the cards in one status across every board of the owner's store: title, project name, owner-friendly state, waiting-reason category and age, bounded by count (50, cap 200) and boards scanned (20), with what the bounds excluded disclosed. Other boards are opened read-only. | `kanban` toolset and the profile named in `kanban.status_report_profiles` |
