...
| `create` / `add` | Create a scheduled job from a prompt, optionally attaching one or more skills via repeated `--skill`. Supports a per-job reasoning pin via `--reasoning-effort <none\|minimal\|low\|medium\|high\|xhigh\|max\|ultra>` and a hard per-run agent-turn limit via `--max-turns 1..500`. |
| `edit` | Update a job's schedule, prompt, name, delivery, repeat count, attached skills, reasoning pin, or per-run turn limit. Supports `--clear-skills`, `--add-skill`, `--remove-skill`, `--reasoning-effort` (empty string clears the pin), and `--max-turns`. |
...
| `create "<title>"` | Create a new task on the active board. Flags: `--body`, `--assignee`, `--parent` (repeatable), `--workspace scratch\|worktree\|dir:<path>`, `--owns <path>` (repeatable literal repository scope), `--read-only`, `--integrates-parent-heads`, `--tenant`, `--priority`, `--triage`, `--idempotency-key`, `--max-runtime`, `--max-retries`, `--skill` (repeatable). Mutating ownership requires an isolated worktree; use `--owns .` for exclusive whole-repository work. |
