...
| `create` / `add` | Create a scheduled job from a prompt, optionally attaching one or more skills via repeated `--skill`. Use `--max-turns 1..500` for a hard per-run agent-turn limit. |
| `edit` | Update a job's schedule, prompt, name, delivery, repeat count, attached skills, or per-run turn limit. Supports `--clear-skills`, `--add-skill`, `--remove-skill`, and `--max-turns`. |
