...
- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_review_findings`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_read`, `kanban_create`, `kanban_link`, `kanban_unblock`, and `kanban_status_report` for profiles named in `kanban.status_report_profiles`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `hermes kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
...
| `kanban_read` | Read a bounded text window of the task's own attachment (by id) or body. | `attachment_id`, `offset`, `limit` |
| `kanban_status_report` | (Allow-listed profiles) list cards in one status across every board, bounded and read-only. | `status`, `limit`, `board_limit` |
